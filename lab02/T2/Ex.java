...
public class Ex {
    ...
    public Ex(String file) throws FileNotFoundException {
        ...
        if (tamanho > 40) {
            System.err.println("Erro: Tamanho máximo é 40x40");
            sc.close();
            System.exit(1);
        }
        ...
        for (int i = 0; i < tamanho; i++) {
            if (Character.isUpperCase(primeira_linha.charAt(i))) {
                this.sopa[0][i] = primeira_linha.charAt(i);
            } else {
                System.err.println("Erro: letra da sopa de letras tem de ser maiuscula");
                System.exit(1);
            }
        }
        ...
        for (int i = 1; i < tamanho; i++) {
            ...
            for (int j = 0; j < tamanho; j++) {
                if (Character.isUpperCase(linha.charAt(j))) {
            ...
            for (int i = 0; i < array_palavras.length; i++) {
                if (array_palavras[i].length() >= 3) {
                    for (int j = 0; j < array_palavras[i].length(); j++) {
                        if (Character.isUpperCase(array_palavras[i].charAt(j))) {
                            System.err.println("Erro: letra das palavras tem de ser minuscula");
                            System.exit(1);
                        }
                    }
                } else {
